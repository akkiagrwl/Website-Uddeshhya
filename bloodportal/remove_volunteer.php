...
<hr />
                              ...
                              <table class="table table-striped table-hover" id="dataTables-example">
                                ...
                                <tbody>
                     <?php $i=1;
							$query="Select Email,Id from login where Type='V'";
								  ...
								  <div class="modal fade bs-example-modal-sm<?php echo $i; ?>" tabindex="-1" role="dialog">
									<div class="modal-dialog modal-sm">
									  <div class="modal-content">
										  ...
										  if($check!=1)
										  {
											  for($k=0;$k<count($st);$k++)echo '<center><b>Assigned State'.($k+1).': </b> '.$st[$k].'<br><br></center>';
										  }
										  else
										  {
											 echo '<center>No States Assigned</center><br><br>'; 
										  }
										  echo '<center><button class="btn btn-success btn-rounded" onclick="return(check());" name="delete" value="'.$id.'"> Delete Volunteer</button><center>'; ?></form></div>
									  </div>
									</div>
								</div>
								  <?php
