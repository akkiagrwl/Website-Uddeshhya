...
<hr />
                              ...
                              <table class="table-striped table-hover col-lg-12" id="dataTables-example">
                                ...
                                <tbody>
                     <?php $i=1;
							$query="Select Id,Name,Age,Sex,BGroup,Address,City,State,pincode,phone,Alternate,email,Detail from donor where Type='AC' and Last_Donated='' Order By State ASC";
							...
							while($row=mysql_fetch_assoc($res))
							{
								...
								$b=$row['City'];
								$c=$row['State'];
								$qry1x="Select State_Name from state where id='$c'";
								$res1x=mysql_query($qry1x) or die(mysql_error());
								$row1x=mysql_fetch_assoc($res1x);
								$c=$row1x['State_Name'];
								$pin=$row['pincode'];
								$phone=$row['phone'];
								$ap=$row['Alternate'];
								  ...
								  <div class="modal fade bs-example-modal-sm<?php echo $i; ?>" tabindex="-1" role="dialog">
									<div class="modal-dialog modal-sm">
									  <div class="modal-content">
										  ...
										  ?>
										  <center><form method="post" action="connection.php"><button class="btn btn-success" name="received" value="<?php echo $id; ?>">Blood Recieved</button>
<button class="btn btn-primary" name="not" value="<?php echo $id; ?>">Unable To Reach</button></form></center></div>
									  </div>
									</div>
								</div>
								  <?php
