...
<head>
     ...
     <meta charset="UTF-8" />
    <title>Acceptors in State</title>
...
<?php if(!isset($_POST['submit']))
	...
	?>
                        ...
                        <div class="panel-heading">
                            <center>Select State To View Acceptors</center>
                        </div>
							...
							$st=$_POST['state'];?>
                              <table class="table-striped table-hover col-lg-12" id="dataTables-example">
								...
								$row1=mysql_fetch_assoc($res1);
								$st=$row1['State_Name'];
								$query="Select Name,Age,Sex,BGroup,Address,City,State,pincode,phone,Alternate,email,Detail from donor where Type='AC' AND State='$id' And Last_Donated=''";
								  ...
								  <div class="modal fade bs-example-modal-sm<?php echo $i; ?>" tabindex="-1" role="dialog">
									<div class="modal-dialog modal-sm">
									  <div class="modal-content">
										  ...
										  echo '<center><b>Detail: </b> '.$ld.'</center><br>';
										  echo '<center><b>Blood Group: </b> '.$bg.'</center><br>';
										  echo '<center><b>Email-Id: </b> '.$email.'</center><br>';
										  ?><center><form method="post" action="connection.php">
										  <button class="btn btn-success" name="received" value="<?php echo $id; ?>">Blood Recieved</button>
<button class="btn btn-primary" value="<?php echo $id; ?>">Unable To Reach</button></form></center>										  </div>
									</div>
								</div>
								  <?php
								  }
								...
								<?php }
									...
									{	
										$query="Select Id,Name,Age,Sex,BGroup,Address,City,State,pincode,phone,Alternate,email,Detail from donor where Type='AC' AND State='$st' AND Last_Donated=''";
							$res=mysql_query($query)or die(mysql_error());
							while($row=mysql_fetch_assoc($res))
							{
								$email=$row['email'];
								  ...
								  <div class="modal fade bs-example-modal-sm<?php echo $i; ?>" tabindex="-1" role="dialog">
									<div class="modal-dialog modal-sm">
									  <div class="modal-content">
										  ...
										  <button class="btn btn-success" name="received" value="<?php echo $id; ?>">Blood Recieved</button>
<button class="btn btn-primary" value="<?php echo $id; ?>">Unable To Reach</button></form>
										</center></div></div>
									  </div>
									</div>
							<?php } ?>
